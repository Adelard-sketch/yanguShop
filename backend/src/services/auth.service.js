...
exports.register = async (data) => {
  ...
  try { await sendVerificationEmail(user.email, code); } catch (err) { console.error('Error sending verification email (register):', err); }
...
exports.resendVerification = async ({ email }) => {
  ...
  await user.save();
  try { await sendVerificationEmail(user.email, code); } catch (err) { console.error('Error sending verification email (resendVerification):', err); }
...
exports.requestPasswordReset = async ({ email }) => {
  ...
  await user.save();
  try { await sendPasswordResetEmail(user.email, token); } catch (err) { console.error('Error sending password reset email', err); }
