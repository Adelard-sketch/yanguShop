...
exports.createChat = async (req, res, next) => {
  try {
    ...
    const newChat = new Chat({
      ...
      guest: !userId
        ? {
            name: req.body.guestName || req.body.name,
            identifier: req.body.guestIdentifier || req.headers['x-guest-id']
          }
        : undefined,
      subject,
    ...
    await newChat.save();
    if (userId) await newChat.populate('user', 'name email');
...
exports.getChatById = async (req, res, next) => {
  try {
    ...
    const chatOwnerId = chat.user ? (chat.user._id ? chat.user._id.toString() : chat.user.toString()) : null;
    if (chatOwnerId && chatOwnerId !== userId.toString() && req.user.role !== 'admin') {
...
exports.addMessage = async (req, res, next) => {
  try {
    ...
    // Check authorization:
    // - If chat is owned by a registered user, require that the caller is that user or an admin/agent
    // - If chat has no registered owner (guest chat), allow anonymous posting
    const chatHasUser = !!chat.user;
    if (chatHasUser) {
      if (!req.user) return res.status(403).json({ message: 'Not authorized' });
      const chatOwnerId = chat.user._id ? chat.user._id.toString() : chat.user.toString();
      if (chatOwnerId !== userId.toString() && req.user.role !== 'admin' && req.user.role !== 'agent') {
        return res.status(403).json({ message: 'Not authorized' });
      }
    }

    // Add user message
    const senderRole = req.user && (req.user.role === 'admin' || req.user.role === 'agent') ? 'agent' : 'customer';
    chat.messages.push({
      sender: senderRole,
      content: message,
      timestamp: new Date()
