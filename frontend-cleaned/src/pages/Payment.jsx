...
export default function Payment() {
  ...
  const [selectedProvider, setSelectedProvider] = useState('mtn'); // for mobile money providers
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const payload = { amount: orderData.total, currency: 'UGX', order: orderData._id, redirectUrl: window.location.origin + '/payment/complete', method: paymentMethod };
      if (paymentMethod === 'momo' || paymentMethod === 'airtel') payload.provider = paymentMethod === 'airtel' ? 'airtel' : selectedProvider;
          ...
          {processingStep === 'input' && (
            <div className="payment-form">
              ...
              <div className="payment-tabs">
                ...
                <button
                  className={`tab ${paymentMethod === 'momo' ? 'active' : ''}`}
                  onClick={() => { setPaymentMethod('momo'); setError(''); }}
                >
                  📱 Mobile Money (MTN/Vodafone)
                </button>
                <button
                  className={`tab ${paymentMethod === 'airtel' ? 'active' : ''}`}
                  onClick={() => { setPaymentMethod('airtel'); setError(''); }}
                >
                  📲 Airtel Money
                </button>
              </div>
              ...
              {paymentMethod === 'card' && (
                ...
                {paymentMethod === 'momo' && (
                  <div className="mobile-form">
                    <div className="providers">
                      <p className="providers-label">Choose Provider</p>
                      <div className="provider-options">
                        <button
                          className={`provider-btn ${selectedProvider === 'mtn' ? 'active' : ''}`}
                        ...
                        <button
                          className={`provider-btn ${selectedProvider === 'vodafone' ? 'active' : ''}`}
                ...
                {paymentMethod === 'airtel' && (
                  <div className="mobile-form">
                    <div className="providers">
                      <p className="providers-label">Airtel Money</p>
                      <div className="provider-options">
                        <div className="provider-btn active">
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        value={mobileNumber}
                        onChange={(e) => setMobileNumber(e.target.value)}
                        placeholder="+233 2XX XXX XXX"
                        className="form-input"
                      />
                    </div>

                    <div className="info-box">
                      <p>💡 You will receive a prompt on your Airtel number to approve the charge.</p>
                    </div>
                  </div>
                )}
