...
const buildProducts = (category, baseProducts, count) =>
  repeatToCount(baseProducts, count).map((item, index) => {
    // build simple reviews array from count number
    const reviewCount = Math.max(1, Math.floor((item.reviews || 5) / 4));
    const reviews = Array.from({ length: reviewCount }).map((_, ri) => ({
      userName: `User ${ri + 1}`,
      rating: Math.max(3, Math.round((item.rating || 4) * 10) / 10),
      comment: `${item.name} is good. (sample review ${ri + 1})`
    }));

    // small shop assignment for some categories
    const shop = ['Electronics', 'Phones'].includes(category)
      ? { _id: 'shop-electro', name: 'Yangu Electronics' }
      : (category === 'Shoes' ? { _id: 'shop-shoeworld', name: 'Shoe World' } : { _id: 'marketplace', name: 'Marketplace' });
    ...
    return {
      ...
      rating: item.rating,
      reviews,
      reviewsCount: item.reviews + index,
      stockCount: 10 + (index % 50),
      inStock: true,
      description: item.description + '\n\nThis is an extended product description with features and specifications. Replace with real content as needed.',
      shop
    };
  });
